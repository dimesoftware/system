...
namespace Dime.System.Tests
{
    ...
    public class EnsureThatTests
    {
        ...
        [TestMethod]
        public void Ensure_That_WithParameter_ConditionAndMessage_ConditionIsTrue_ShouldNotThrowException()
            => Ensure.That<ArgumentNullException>(condition: 1 == 1, message: "Items must be equal", paramName: "parameter1");

        [TestMethod]
        public void Ensure_That_ConditionAndMessage_ConditionIsFalse_ShouldThrowException()
            => Assert.ThrowsException<Exception>(() => Ensure.That(1 == 2, "Items must be equal"), "Items must be equal");
